...
plot_roc("Step",preds_ob2_lr,0.2,0.8,T)
```
```{r}
...
final_rf = randomForest(fracture ~., data=train, mtry=8, ntree=8000,importance=T)
final_rf
```


```{r}
rpartPred_test <- predict(final_rf, test, type="prob")
rpartPred_test[,2]
```

```{r}
#rpartPred_test
pred_rf = predict(rpartPred_test[,2], test$fracture)
pred_rf
#rpartPred_test
#test_pred_obs = data.frame(obs=test$fracture, pred=rpartPred_test)
#test_pred_obs
```

```{r}
roc.perf = performance(rpartPred_test, measure = "tpr", x.measure = "fpr")
auc.train <- performance(rpartPred_test, measure = "auc")
...
#result.roc <- roc(test_pred_obs$obs, rpartPred_test$No) 
```


```{r}
library(MLmetrics)
confusionMatrix(test_pred_obs$pred, test_pred_obs$obs, "Yes", mode = "prec_recall")
postResample(pred = rpartPred_test, obs = test$fracture)
prSummary(data = test_pred_obs, lev= c("Yes","No"), model=rpart_fit)
```
```
